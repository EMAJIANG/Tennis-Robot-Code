...
class RacketControl(Node):
    def __init__(self):
        ...
        self.joint_positions = np.array([0.0, 0.0, 0.0, -30 / 180 * 2 * math.pi])  # initialize joint position , rad to angle 
        self.init_hitangle=self.joint_positions[-1] # initial racket hitting angle 
    ...
    def hit_racket(self):
        action = ArticulationAction(
            joint_positions=[self.joint_positions[1], self.joint_positions[0], self.joint_positions[2], 0 / 180 * 2 * math.pi],
            joint_velocities=[0, 0, 0, 3]
        )
    ...
    def recover_racket(self):
        action = ArticulationAction(
        joint_positions=[self.joint_positions[1], self.joint_positions[0], self.joint_positions[2], -30 / 180 * 2 * math.pi],
        joint_velocities=[0, 0, 0, 3]
        )
        self.TR_V4.apply_action(action)
