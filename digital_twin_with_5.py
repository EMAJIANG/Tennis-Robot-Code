...
RACKET_2_DEGREE = 7618.62 # 1度对应的脉冲
RACKET_POSITION_LIMIT = 120 # 120度范围
CONTROLWORD = 0x6040
STATUSWORD = 0x6041
...
class TRJointSubscriber(Node):
    ...
    def joint_callback(self, msg: JointState):
        ...
        target_r_degrees = int(mapped_r * RACKET_2_DEGREE)
        ...
        self.get_logger().info(
            f"Received JointState -> mapping to counts X:{target_x_counts}, Y:{target_y_counts}, Z:{target_z_counts} ,R:{target_r_degrees} and replaced trajectory queue"
        )
...
class MinimalExample:
    ...
    A6_PRODUCT_CODE = 0x60380007
    RACKET_PRODUCT_CODE =  0x00030924
    ...
    def __init__(self, ifname):
        ...
        self._expected_slave_mapping = {0: SlaveSet('X1', self.A6_PRODUCT_CODE, self.elmo_setup),
                                        1: SlaveSet('X2', self.A6_PRODUCT_CODE, self.elmo_setup),
                                        2: SlaveSet('Y', self.A6_PRODUCT_CODE, self.elmo_setup),
    ...
    def enable_motor(self, slave_pos):
        ...
        if slave_pos < 2:
            ...
            slave.sdo_write(PROFILE_ACCELERATION, 0, struct.pack('<I', profile_acceleration))
            slave.sdo_write(PROFILE_DECELERATION, 0, struct.pack('<I', profile_deceleration))
           ####################### todo : define the acc and dec of the Racket motor ###############
    ...
    def run_trajectory(self):
        ...
        slave_r = self._master.slaves[4]
        all_slaves = [slave_x1, slave_x2, slave_y, slave_z, slave_r]
            ...
            while True:
                ...
                if newest is not None:
                    ...
                    if (current_target_point is None) or (
                        ...
                        newest['z'] != current_target_point['z'] or
                        newest['r'] != current_target_point['r']
                    ):
                        current_target_point = newest.copy()
                        last_point_time = time.time()
                        print(f"\n[{last_point_time:.2f}s] New Target Point -> X: {current_target_point['x']}, Y: {current_target_point['y']}, Z: {current_target_point['z']},R: {current_target_point['r']}")
                        ...
                        slave_r.sdo_write(TARGET_POSITION, 0, struct.pack('<i', current_target_point['r']))
                ...
                try:
                    ...
                    input_r_as_int16 = struct.pack('<iIH', current_target_point['r'], 0x01, 0x0F)
                    self._master.slaves[4].output = input_r_as_int16
                except Exception as e:
                ...
                try:
                    output_r_as_bytes = self._master.slaves[4].input
                    output_r_as_int16 = struct.unpack('<i I H ', output_r_as_bytes)
                    r_actual_pos = output_r_as_int16[0]
                    r_digital_inputs = output_r_as_int16[1]
                    r_status_word = output_r_as_int16[2]
                except Exception:
                ...
                if current_target_point is not None:
                    if (abs(x1_actual_pos - current_target_point['x']) < 1000 and
                        ...
                        abs(z_actual_pos - current_target_point['z']) < 500 and
                        abs(r_actual_pos - current_target_point['r']) < 700 ):
